...
public class GPSTracker extends Service implements LocationListener {
    ...
    public Location getLocation() {
        try {
            Log.i("No perm Try:","Getting PErmission1");
            locationManager = (LocationManager) mContext
                    .getSystemService(LOCATION_SERVICE);
            // getting GPS status
            isGPSEnabled = locationManager
                    .isProviderEnabled(LocationManager.GPS_PROVIDER);
            Log.i("GPS Status","status is"+isGPSEnabled);
            // getting network status
            isNetworkEnabled = locationManager
                    .isProviderEnabled(LocationManager.NETWORK_PROVIDER);
            Log.i("Network Status","status is"+isNetworkEnabled);
            if (!isGPSEnabled && !isNetworkEnabled) {
                // no network provider is enabled
                Log.i("Nothing Enabled","No providers enabled");
            } else {
                ...
                if (isNetworkEnabled) {
                    if(ContextCompat.checkSelfPermission((Activity)mContext, Manifest.permission.ACCESS_FINE_LOCATION)!= PackageManager.PERMISSION_GRANTED) {
                        ...
                        Log.i("No perm:","Getting PErmission1 Network");
                    }
                ...
                if (isGPSEnabled) {
                    if(ContextCompat.checkSelfPermission((Activity)mContext, Manifest.permission.ACCESS_FINE_LOCATION)!= PackageManager.PERMISSION_GRANTED) {
                        ActivityCompat.requestPermissions((Activity)mContext, new String[]{Manifest.permission.ACCESS_FINE_LOCATION},1);
                        Log.i("No perm:","Getting PErmission2 GPS enabled");
                    }
                    if (location == null) {
