...
/**
 * Created by Sonu on 24/03/16.
 */
public class RestaurantDataJSON
{

  private static final String TAG = "RestaurantDataJSON";
  public List<Map<String, ?>> restaurantList;
  ...
  private HashMap createRestaurant(String name, String description, double rating, String url, String id)
  {
  ...
  private HashMap createRestaurant(String name, String address, String dispAddress, String categories, double rating, String reviews, String distance, String url, String id)
  {
  ...
  private HashMap createESRestaurant(String name, String address, String dispAddress, String categories, String minDel, String delPrice, boolean isOpen, String url, String id)
  {
    HashMap restaurant = new HashMap();
    restaurant.put("name", name);
    restaurant.put("address", address);
  ...
  private void createRestaurants(String jsonString)
  {
    try
    {
      ...
      for (int i = 0; i < array.length(); i++)
      {
        ...
        if (obj != null)
        {
          ...
          restaurantList.add(createRestaurant(name, desc, rating, url, id));
        }
      }
    }
  ...
  public HashMap downloadRestaurantDetailJSON(String urlString)
  {
    ...
    {
      logMessage("Exception in downloadMovieDataJson");
      e.printStackTrace();
    }
    return detailMap;
  }
  ...
  private HashMap buildDetailJSON(String apiKey, String name, String dispAddress, String address, String categories, String imgUrl, boolean isOpen, double latitude, double longitude, int minWaitTime, int maxWaitTime, String id, HashMap daysMap, String delivery, String min)
  {
    HashMap restaurant = new HashMap();
  ...
  private void createRestaurantDetailed(String jsonString)
  {
    HashMap map = null;
    try
    {
      ...
      {
        JSONObject obj = (JSONObject) array.get(i);
        String minDelivery = "Minimum delivery - NA";
        String delPrice = "Delivery Charges - NA";
        if (obj != null)
        {
          String name = (String) obj.get("name");
          String street = (String) obj.get("streetAddress");
          ...
          String categories = createCategories(cat_list);
          String imgUrl = (String) obj.get("logoUrl");
          if (obj.has("deliveryMin"))
          {
            double minimumDel = (double) obj.get("deliveryMin");
            minDelivery = "Minimum delivery - $" + String.valueOf(minimumDel);
          }
          if (obj.has("deliveryPrice"))
          {
            double deliveryPrice = (double) obj.get("deliveryPrice");
            delPrice = "Delivery Charges - $" + String.valueOf(deliveryPrice);
          }
          boolean isOpen = (boolean) obj.get("open");
          String id = (String) obj.get("apiKey");
          restaurantList.add(createESRestaurant(name, address, dispAddress, categories, minDelivery, delPrice, isOpen, imgUrl, id));
          //map = createMovieDetail(name, desc, year, length, rating, director, stars, url);
        }
      }
    }
    catch (Exception e)
  ...
  public String downloadMovieDataJSON(String urlString)
  {
    String jsonString = null;
    try
    {
  ...
  public void downloadMovieDetailJSON(String urlString)
  {
    String jsonString = null;
    try
    {
      ...
      if (httpURLConnection.getResponseCode() == HttpURLConnection.HTTP_OK)
      {
        InputStream stream = httpURLConnection.getInputStream();
        jsonString = getStringFromStream(stream);
        logMessage("Json is:" + jsonString.toString());
        createRestaurantDetailed(jsonString);
      }
    }
    catch (Exception e)
    {
  ...
  {
    int i, movieSize = restaurantList.size();
    for (i = 0; i < movieSize; i++) if (restaurantList.get(i).get("name").toString().toLowerCase().contains(query.toLowerCase())) return i;
    return movieSize;
  }
